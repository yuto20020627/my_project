...
out = cv2.VideoWriter('output_with_line.avi', fourcc, 30.0, (output_width, output_height))  # 出力サイズを設定
...
def process_video():
    ...
    while cap.isOpened():  # 動画が開かれているか
        ...
        resized_fg_mask = cv2.resize(fg_mask, (output_width, output_height))
        combined_frame = cv2.hconcat([resized_frame, cv2.cvtColor(resized_fg_mask, cv2.COLOR_GRAY2BGR)])
        print("Frame size (after resize):", combined_frame.shape)

        # フレームサイズの確認
        print(f"Frame size: {combined_frame.shape}")

        # 動画として出力
        out.write(combined_frame)  # フレームを動画として保存
