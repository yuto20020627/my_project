...
def process_video():
    ...
    while cap.isOpened():
        ...
        if len(history) > 60: # 履歴を60フレーム分に制限
            history.pop(0)

        # 確定線の更新(過去60フレームの数値が誤差+-5なら更新)
        if len(history) == 60 and max_right_x != frame_width:
            count = 0
            for i in range(1,60):
                if abs(history[i] - history[0]) <= 5:
                    count = count + 1
            if count == 59:
...
def get_next_bus_time(times):
    ...
    # #scheduleのtimeをdatetime型に変換し、現在時刻以降すべての時刻を配列として取得
...
def get_bus_time():
    ...
    next_bus_time = get_next_bus_time(bus_schedule["bus_stops"][0])

    return jsonify({
        'line': confirmed_line_x,
