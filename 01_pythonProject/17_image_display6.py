...
THRESHOLD_DIFF = 5  # 確定線更新のしきい値（許容誤差）
...
def process_video():
    ...
    while cap.isOpened():
        ...
        ######確定線の更新#################################
        # 位置履歴に現在の位置を追加
        history.append(max_right_x)  # .appendで配列の末尾に追加
        if len(history) > HISTORY_MAX_SIZE:
            history.pop(0)

        if len(history) == HISTORY_MAX_SIZE and max_right_x != frame_width:
            count = 0
            for i in range(1,HISTORY_MAX_SIZE): #HISTORY_MAX_SIZE = 30
                if abs(history[i] - history[0]) <= THRESHOLD_DIFF: #THRESHOLD_DIFF = 5
                    count = count + 1
            if count == HISTORY_MAX_SIZE - 1:
