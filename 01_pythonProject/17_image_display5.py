...
def process_video():
    ...
    while cap.isOpened():
        ...
        # 確定線（赤線）の更新
        if len(history) == HISTORY_MAX_SIZE and max_right_x != frame_width and all(
                abs(history[i] - history[i + 1]) <= THRESHOLD_DIFF for i in range(HISTORY_MAX_SIZE - 1)):
            confirmed_line_x = max_right_x  # THRESHOLD_DIFF = 0
        ##################################################
